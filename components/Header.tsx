      ...
      <div className="container-page flex h-20 items-center justify-between gap-8">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-7 lg:flex">
          {links.map(([label, href]) => (
            <Link key={href} href={href} className="text-[13px] font-medium opacity-90 hover:opacity-60">
              {label}
            </Link>
          ))}
        ...
        <button
          className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold lg:hidden"
          aria-expanded={open}
          aria-controls="mobile-navigation"
          onClick={() => setOpen((current) => !current)}
        >
          Menu <span className="text-azael-gold-bright">{open ? "−" : "+"}</span>
        </button>
      </div>
      {open ? (
        <nav id="mobile-navigation" className="mx-3 border border-white/15 bg-azael-navy-deep/95 p-6 text-white shadow-2xl backdrop-blur lg:hidden">
          <div className="flex flex-col gap-5">
            {links.map(([label, href]) => (
              <Link key={href} href={href} onClick={() => setOpen(false)} className="text-lg font-medium">
                {label}
              </Link>
            ))}
